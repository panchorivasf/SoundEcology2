...
bbai_folder2 <- function(folder = NULL,
  ...
  # Define parallel computation with error handling for corrupted files
  results <- foreach(file = audio.list,
                     .packages = c("tuneR", "seewave", "tibble"),
                     .errorhandling = "pass") %dopar% {
                       tryCatch({
                         ...
                         median_size <- median(file.info(audio.list)$size / 1024
                                               ...
                                               return(tibble(file_name = filename) |> bind_cols(results))
  ...
  # Filter out NULL results from corrupted files
  valid_results <- results[!sapply(results, is.null)]
  
  if (length(valid_results) == 0) {
    stopCluster(cl)
    stop("No valid files remaining after filtering corrupted files.")
  }
  
  # Combine all the valid results into a single tibble
