...
#' @param wave A wave object imported with readWave().
...
fadi <- function(wave,
  ...
  threshold.fixed <- as.numeric(threshold.fixed)
  NEM <- as.numeric(NEM)
  ...
  if (is.numeric(as.numeric(threshold.fixed))){
    threshold.fixed<- as.numeric(threshold.fixed)
  } else{
    stop(" threshold.fixed is not a number.")
  }

  if (is.numeric(as.numeric(freq.step))){
    freq.step <- as.numeric(freq.step)
  } else{
    stop(" freq.step is not a number.")
  }
  ...
  if (wave@stereo == TRUE) {
    ...
    if (NEM==1){     #if noise.file should be provided
      if (is.null(noise.file)){     #parameter validation
        cat(paste("\n WARNING: \n No input noise file .\n Then histogram-based noise-estimation algorithms will be used to estimate the noise of the recording. \n\n"))
        NEM=2
      }else{
        if(length(noise.file)/noise.file@samp.rate<5){
          stop("\n Warning: \n The input noise file length is insufficient. \n\n")
        }

        noise_left<-channel(noise.file, which = c("left"))
        noise_right<-channel(noise.file, which = c("right"))
    ...
    if (NEM==2){  #if noise.file is omitted

      if(length(wave)/wave@samp.rate < 30){
        stop("\n WARNING: \n The input wave length is insufficient. \n\n")
      }

      noise_db_left<-noise_estimation(specA_left,max.freq,freq_per_row)
      noise_db_right<-noise_estimation(specA_right,max.freq,freq_per_row)
    }

    Freq <- seq(from = 0, to = max.freq - freq.step, by = freq.step)

    #left channel
    threshold_left=db_threshold(specA_left,noise_db_left,min.freq,max.freq,freq_per_row,threshold.fixed,gamma)

    Score <- rep(NA, length(Freq))
    Scorez <- rep(NA, length(Freq))

    for (j in 1:length(Freq)) {
      Scorez[j] = getscore(specA_left, Freq[j], (Freq[j] + freq.step), threshold_left, freq_per_row)
    }
    ...
    #right channel
    threshold_right=db_threshold(specA_right,noise_db_right,min.freq,max.freq,freq_per_row,threshold.fixed,gamma)
    Score <- rep(NA, length(Freq))
    Scorez <- rep(NA, length(Freq))

    for (j in 1:length(Freq)) {
      Scorez[j] = getscore(specA_right, Freq[j], (Freq[j] + freq.step), threshold_right, freq_per_row)
    }
    Score= Scorez/sum(Scorez)
    ...
    for (j in seq(length(Freq), 1, by = -1)) {
      # 			cat(paste(Freq[j], "-", (Freq[j]+freq.step), ",", round(left_vals[j],6), ",", round(right_vals[j],6), "\n", sep=""))
      left_bandvals_return[j] = round(left_vals[j], 6)
      right_bandvals_return[j] = round(right_vals[j], 6)
      left_bandrange_return[j] = paste((Freq[j]/1000), "-", ((Freq[j]/1000) + (freq.step/1000)), " kHz", sep = "")
      right_bandrange_return[j] = paste((Freq[j]/1000), "-", ((Freq[j]/1000) + (freq.step/1000)), " kHz", sep = "")
    }
    ...
    fadiOutputStereo <- fadiOutputStereo |>
      add_column(wlen = wlen,
                 wfun = "hanning",
                 dbth = threshold.fixed,
                 minf = min.freq,
                 maxf = max.freq,
                 nbands = (max.freq/freq.step),
                 norm = "FALSE",
                 noisered = noisered,
                 rmoff = "false",
                 propden = "within band",
  ...
  else {      #if input wave is monochannel
    ...
    #matrix of values
    # cat("\n Calculating index. Please wait... \n\n")
    specA_left <- seewave::spectro(wave, f = samplingrate, wl = wlen, plot = FALSE,norm=FALSE,dB=NULL,unit="power")$amp
    specA_left<-specA_left^2
    ...
    for (j in seq(length(Freq), 1, by = -1)) {
      # 			cat(paste(Freq[j], "-", (Freq[j]+freq.step), ",", round(left_vals[j],6), "\n", sep=""))
      left_bandvals_return[j] = round(left_vals[j], 6)
