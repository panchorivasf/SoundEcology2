...
#'
#' @param df A data frame containing acoustic indices. Expected column names depend on the `origin` parameter. For `"kaleidoscope"` data, expected columns include `in.file`, `channel`, and various acoustic indices (`ndsi`, `cent`, etc.). For `"soundecology2"` data, columns should include `file_name`, `sensor_id`, and `datetime`.
#' @param origin A character string specifying the source of the data. Options are `"kaleidoscope"` or `"soundecology2"`. Default is `"kaleidoscope"`.
#'
#' @return A data frame containing summary statistics (`mean`, `median`, `mode`, `max`, `min`) for each acoustic index, with the associated recording file closest to each statistic.
#' @export
#'
#' @examples
...
indices_summary <- function(df, origin = "soundecology2") {
  ...
  if (origin == "kaleidoscope") {
    ...
    df <- dplyr::select(df, c(in.file, unit_id, datetime, date, hour, dplyr::everything()))

  } else if (origin == "soundecology2") {
    # Add this line before dplyr::reframe
    print(unique(df$index))
    
    # Adjust to the structure of `indices_soundec2`
    df <- df %>%
      dplyr::rename(in.file = file_name,
                    unit_id = sensor_id) %>%
  ...
  summary <- df %>%
    dplyr::group_by(unit_id, channel, index) %>%
    dplyr::reframe(
      ...
      mode_value = if (length(DescTools::Mode(value)) == 1) DescTools::Mode(value) else NA_real_,
      closest_to_mode = if (!is.na(mode_value)) in.file[which.min(abs(value - mode_value))] else NA_character_
    )
