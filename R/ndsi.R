...
ndsi<- function(wave,
  ...
  if (wave@stereo == TRUE) {
    ...
    left3 <- data.frame(matrix(NA, nrow = wlen/2, ncol = floor(duration)))

    left4 <- apply(left2, 2, pwelch, fs = samplingrate, nfft = wlen, plot = FALSE)
    ...
    right3 <- data.frame(matrix(NA, nrow = wlen/2, ncol = floor(duration)))

    right4 <- apply(right2, 2, pwelch, fs = samplingrate, nfft = wlen, plot = FALSE)
