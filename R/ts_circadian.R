...
ts_circadian <- function(data, 
  ...
  # Apply the selected scaling/transformation to the grouped data
  if (scaling == "minmax") {
  ...
  for (idx in unique(data_grouped$index)) {
    ...
    if (loess) {
      ...
      loess_model <- loess(value_avg ~ as.numeric(hour), data = data_idx, 
                           span = span)
      ...
      plot <- plot |>
        add_trace(data = data_idx, x = ~hour, y = smoothed_values, 
                  ...
                  showlegend = TRUE,
                  hovertemplate = paste('Index:', idx, '<br>',
                                        'Time: %{x}<br>',
  ...
  if (scaling == "none") {
    ...
    plot <- plot |> layout(yaxis = list(title = "Value"))
    
    
  } else {
    plot <- plot |> layout(yaxis = list(title = paste("Scaled Value (", scaling, ")")))
  ...
  #     #title = plot.title,
  #          # xaxis = list(title = "Hour of the Day", tickangle = 90),
  #          yaxis = list(title = ifelse(scaling == "none", "Value", paste("Scaled Value (", scaling, ")"))))
